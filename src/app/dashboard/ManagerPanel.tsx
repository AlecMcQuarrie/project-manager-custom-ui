...
export default function ManagerPanel() {
  ...
  const occupiedUnits = demoUnits.filter(
    (unit) => unit.status === "occupied"
  ).length;
  const vacantUnits = demoUnits.filter(
    (unit) => unit.status === "vacant"
  ).length;
  const maintenanceUnits = demoUnits.filter(
    (unit) => unit.status === "maintenance"
  ).length;

  const totalResidents = demoUsers.filter((u) => u.role === "resident").length;
  const pendingBills = demoBills.filter(
    (bill) => bill.status === "pending"
  ).length;
  const overdueBills = demoBills.filter(
    (bill) => bill.status === "overdue"
  ).length;
  const pendingMaintenance = demoMaintenanceRequests.filter(
    (mr) => mr.status === "pending"
  ).length;
  const inProgressMaintenance = demoMaintenanceRequests.filter(
    (mr) => mr.status === "in-progress"
  ).length;
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-gray-400">
                          Unit{" "}
                          {demoUnits.find((u) => u.id === request.unitId)?.id} •{" "}
                  ...
                  {recentBills.map((bill) => (
                    <div key={bill.id} className="flex items-start space-x-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-gray-400">
                          Unit {demoUnits.find((u) => u.id === bill.unitId)?.id}{" "}
                          • ${bill.amount}
                        </p>
                      </div>
                      <div className="flex-shrink-0">
            ...
            <CardContent>
              <div className="space-y-4">
                {recentActivity.map((activity) => (
                  <div
                    key={activity.id}
                    className="flex items-center space-x-3 p-3 border border-[#22304a] rounded-lg bg-[#22304a]"
                  >
                    <div className="flex-shrink-0">
