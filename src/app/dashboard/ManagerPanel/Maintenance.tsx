...
import { User as UserType, Unit, MaintenanceRequest } from "../../../lib/types";
import { demoUsers, demoUnits, demoMaintenanceRequests } from "@/lib/data";
...
export default function Maintenance() {
  ...
  const stats = [
    ...
    {
      name: "In Progress",
      value: maintenanceRequests.filter((r) => r.status === "in-progress")
        .length,
      icon: AlertTriangle,
            ...
            <CardContent>
              <div className="flex items-center">
                <div className="flex-shrink-0">
                  <stat.icon
                    className={`h-6 w-6 ${stat.color || "text-blue-400"}`}
                  />
                </div>
            ...
            <table className="min-w-full divide-y divide-[#22304a]">
              ...
              <tbody className="bg-[#181c23] divide-y divide-[#22304a]">
                {filteredRequests.map((request) => (
                  <tr
                    key={request.id}
                    className="hover:bg-[#22304a] transition-colors duration-200"
                  >
          ...
          <div className="space-y-4">
            {maintenanceRequests
              ...
              .map((request) => (
                <div
                  key={request.id}
                  className="flex items-center justify-between p-3 border border-[#22304a] rounded-lg bg-[#22304a]"
                >
